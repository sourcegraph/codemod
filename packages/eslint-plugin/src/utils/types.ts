// Utilities copied from @typescript-eslint-plugin without changes.
...
export function getTypeName(typeChecker: ts.TypeChecker, type: ts.Type): string {
    ...
    if ((type.flags & ts.TypeFlags.TypeParameter) !== 0) {
        ...
        const typeParameterDecl = decls?.[0] as ts.TypeParameterDeclaration
        if (ts.isTypeParameterDeclaration(typeParameterDecl) && typeParameterDecl.constraint) {
            return getTypeName(typeChecker, typeChecker.getTypeFromTypeNode(typeParameterDecl.constraint))
        }
    }
...
export function isUnsafeAssignment(
    ...
    */
        ...
        if (
            ...
            (senderNode.typeParameters === null || senderNode.typeParameters === undefined)
        ) {
            // special case to handle `new Map()`
            // unfortunately Map's default empty constructor is typed to return `Map<any, any>` :(
            // https://github.com/sourcegraph/codemod/issues/2109#issuecomment-634144396
            return false
        }
        ...
        for (let index = 0; index < typeArguments.length; index += 1) {
            ...
            const receiverArgument = receiverTypeArguments[index]

            const unsafe = isUnsafeAssignment(argument, receiverArgument, checker, senderNode)
            if (unsafe) {
                return { sender: type, receiver }
            }
        }
